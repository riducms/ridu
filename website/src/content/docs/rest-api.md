...
description: 'Call Ridu’s generated HTTP API from any language or Fetch-compatible runtime.'
product: data
...
token, or an API key as described in [Authentication and actor routes](/docs/rest-api/authentication/).

## Find the route you need {#routes}

| Goal                                                                | Route family or guide                                                                                            |
| ------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| List, create, read, update, or delete collection documents          | `/api/collections/{collection}` — [Collections, globals, and files](/docs/rest-api/collections/)                 |
| Read or update a global                                             | `/api/globals/{global}` — [Collections, globals, and files](/docs/rest-api/collections/)                         |
| Use trash, versions, publishing, scheduling, or uploads             | [Collections, globals, and files](/docs/rest-api/collections/)                                                   |
| Log in, manage sessions or API keys, or inspect the current actor   | [Authentication and actor routes](/docs/rest-api/authentication/)                                                |
| Send filters, selection, population, sorting, locale, or pagination | [Queries, responses, and errors](/docs/rest-api/queries/)                                                        |
| Add an application-owned route                                      | [Custom endpoints](/docs/custom-endpoints/)                                                                      |
| Call an installed plugin transport                                  | Inspect its generated OpenAPI operation and plugin guide, such as [GraphQL](/docs/graphql/) or [MCP](/docs/mcp/) |
...
same Ridu error envelope. See [Custom endpoints](/docs/custom-endpoints/) for route placement,
...
| --------------------------------------- | ------------------------------------------------- | ------------------------------------------------------------------------ |
| Go code in the Ridu process             | [Local Go API](/docs/local-api/)                  | Avoids a loopback HTTP request while preserving the operation engine.    |
| TypeScript                              | [Generated TypeScript SDK](/docs/typescript-sdk/) | Encodes queries, checks envelopes, and returns generated document types. |
...
the session cookie. See [CORS](/docs/cors/) for origins, headers, proxy trust, and preflight errors.
...
reporting a failed request. Continue with [Queries, responses, and errors](/docs/rest-api/queries/)
