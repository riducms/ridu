---
title: 'GraphQL'
description: 'Add an optional manifest-derived GraphQL endpoint with typed resources, shared authorization, and bounded execution.'
product: plugins
eyebrow: 'Plugins'
order: 200
aliases:
...
availability:
  status: experimental
  label: 'Opt-in transport'
  description: 'The generated schema, CRUD/auth/localization/version/trash paths, safeguards, and extension fields work; broader pinned Payload conformance is still being widened.'
  anchor: limits
navigation:
  section: 'Extend Ridu'
  parent: 'plugins'
...
Typed filter inputs compile into Ridu's finite [query vocabulary](/docs/querying/). Nested group,
...
carry the byte source or server-owned metadata. Use multipart/remote [upload APIs](/docs/uploads/)
...
See the [`graphql` plugin reference](/reference/graphql/), [REST API](/docs/rest-api/), and
[TypeScript SDK](/docs/typescript-sdk/). Use [Capability status](/docs/status/) to track the
