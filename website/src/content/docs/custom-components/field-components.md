...
description: 'Replace a field input with your own Svelte component, starting with a title that counts characters.'
product: admin
order: 151
...
[`defineFieldEditor`](/reference/plugin/define-field-editor/) returns the registration stored in
the `fields` map. [`defineAdmin`](/reference/plugin/define-admin/) collects that registration with
...
`decodeConfig`. [FieldEditorProps](/reference/plugin/field-editor-props/) documents the value,
...
`Collections` list, as shown in the [Quickstart](/docs/quickstart/). If Posts already exists, keep
...
Add [`.LiveValidate(...)`](/docs/fields/live-validation/) to the Go field to enable server
...
To change an array row's heading, use [Row labels](/docs/custom-components/row-labels/).
Selects, relationships, uploads, JSON, and rich text cannot use a component registered this way.
Their custom editors need a [plugin](/guides/custom-fields/).
