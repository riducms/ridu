---
title: 'Field hooks'
description: 'Trim input, change typed values, and format responses with hooks attached to a field.'
product: core
eyebrow: 'Hooks'
order: 83
navigation:
...
[`.DefaultFrom(...)`](/docs/fields/defaults/). Use a hook when existing or explicitly supplied
values also need to change.

New to `operation.Value[T]`, `Keep`, or `Replace`? [Operations and callbacks](/docs/go-packages/operation/)
explains what those values mean and how to return them from a field hook. For raw
`store.Value` data, see [Documents and values](/docs/go-packages/store/).
...
[when hooks run](/docs/hooks/collections/#lifecycle).
...
[Using other field values](/docs/fields/callback-values/) for nested fields, previous values,
...
[TextList and NumberList](/docs/fields/lists/#behavior) use `[]string` and `[]float64`;
...
replace it. See [`field.Hooks`](/reference/field/hooks/),
[`field.Transform`](/reference/field/transform/), and
[`field.Observer`](/reference/field/observer/) for their types.
...
[collection or global hook](/docs/hooks/context/#stored-and-returned-values) when a change needs
to update several fields together. See [Hook context](/docs/hooks/context/) for the arguments
...
[Live server validation guide](/docs/fields/live-validation/) shows how to share the rule between
both callbacks.

For large arrays, Blocks, or plugin-declared embedded content, continue with
[Hook and value performance](/docs/performance/hooks-and-values/) to keep per-occurrence callbacks
