...
Collection and global hooks receive [`ridu.HookContext`](/reference/ridu/hook-context/).
...
Values in `Data` use [`store.Value`](/reference/store/value/). Read a string with
[`StringValue()`](/reference/store/value-string-value/) and write one with
[`store.String(...)`](/reference/store/string/). [Documents and values](/docs/go-packages/store/)
...
[Schema and identifiers](/docs/go-packages/schema/#identifiers).
...
A [field hook](/docs/hooks/fields/) receives `ctx` plus a separate `operation.Value[T]` for its
...
See [Using other field values](/docs/fields/callback-values/) for complete examples covering
...
[Save related documents together](/docs/hooks/transactions-and-errors/#nested-operations) for
