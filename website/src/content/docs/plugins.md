---
title: 'Plugins'
description: 'Install reusable features or build a plugin with Go behavior, new field types, and admin components.'
product: plugins
eyebrow: 'Plugins'
order: 170
navigation:
...
start with [Custom components](/docs/custom-components/). You can register application components
directly. The guide below is for installing or building plugins that add server behavior or new
field types. For database connections and file storage, use [adapters](/docs/adapters/).
...
conflicts before the application starts. [Build a field plugin](/guides/custom-fields/) shows a
...
Create the JavaScript export with [defineAdminPlugin](/reference/plugin/define-admin-plugin/).
Use [definePluginField](/reference/plugin/define-plugin-field/) for each new field type's default
editor, or [defineFieldComponent](/reference/plugin/define-field-component/) for an alternative
editor selected by an application. Each reference page lists its options and includes a working
registration example. [Build a field plugin](/guides/custom-fields/) connects them to a Go plugin
...
Official plugins include [Rich text](/docs/rich-text/), [SEO](/docs/seo/),
[Form Builder](/docs/form-builder/), and [GraphQL](/docs/graphql/).
For a complete new field type, follow [Build a field plugin](/guides/custom-fields/).
