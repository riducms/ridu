...
description: 'Add your own rules to a field and show useful messages when a value cannot be saved.'
product: core
...
For messages while someone is typing, see [Live server validation](/docs/fields/live-validation/).
Register both `.Validate(...)` and `.LiveValidate(...)` when the rule should run at both times.

[Operations and callbacks](/docs/go-packages/operation/) explains `operation.Value[T]`,
...
[Using other field values](/docs/fields/callback-values/).

To change a value before saving, use a [field hook](/docs/hooks/fields/#normalize-input).
...
For target methods and types, see the [operation API reference](/reference/operation/).
...
The [Live server validation guide](/docs/fields/live-validation/) walks through a complete
...
and save hooks have not run. See [callback arguments](/docs/fields/live-validation/#live-context).

### Understand when feedback appears {#live-timing}

Checks start after editing a field and wait 500 ms after the last edit. See
[timing and feedback](/docs/fields/live-validation/#live-timing) for retries, skipped checks,
...
[rows and embedded forms](/docs/fields/live-validation/#live-embedded) for context and Apply behavior.
...
[TextList and NumberList](/docs/fields/lists/#behavior) validators receive the whole list as
