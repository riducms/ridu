...
builds filters for [access rules](/docs/access-control/).
...
[`NewPath`](/reference/query/new-path/) names each field. `query.Number(maxPrice)` and
`query.Boolean(true)` give the comparisons their values. `query.And` joins the two conditions:
a document must satisfy both.

The helper returns a [`query.Expression`](/reference/query/expression/). Keep it in a variable,
...
A [`query.Path`](/reference/query/path/) represents one field location. It is a validated Go value,
...
is not part of a field name. Use [`ParsePath`](/reference/query/parse-path/) for a dotted string,
...
caller-supplied filters; see [protected field queries](/docs/access-control/#protected-field-queries).
...
[`query.Value`](/reference/query/value/) is the value **inside a comparison**. Construct it with the
...
[Documents and values](/docs/go-packages/store/) for working with document data.
...
[`query.Compare`](/reference/query/compare/), which returns an error for invalid combinations.
For example, `OperatorExists` expects `query.Boolean(...)`, while `OperatorIn` expects a list.
The [operator table](/docs/querying/#where) explains the available comparisons and text matching.
...
[`ridu.Where`](/reference/ridu/where/) to allow access only to matching documents.
...
[passing the caller](/docs/local-api/#actors).
...
[Querying data](/docs/querying/#where).
...
See [sorting](/docs/querying/#sort) and [pagination](/docs/querying/#pagination) for request limits,
ordering, and result metadata. The [Local Go API](/docs/local-api/) covers the surrounding read
options, while the [`query` reference](/reference/query/) lists each constructor and its arguments.
