...
description: 'See which Ridu capabilities are available, limited, experimental, or planned in the current release.'
product: core
eyebrow: 'Start here'
order: 15
...
- **MongoDB deployment.** Production support covers only generated starter and blank projects on
  ...
  scoped credentials for the application, verification, and backups. See [MongoDB](/docs/mongodb/).
...
The [GraphQL plugin](/docs/graphql/) is opt-in and supports generated schema, PostgreSQL CRUD,
localization, filters, population, and bounded requests. Compatibility may still change before the
plugin is marked available.

The [MCP plugin](/docs/mcp/) is opt-in and exposes only explicitly selected collection and global
...
1. [Install Ridu](/docs/installation/) and exercise the generated project.
2. Read [Core concepts](/docs/core-concepts/) and [Project structure](/guides/project-structure/) to
...
5. Use [Security](/docs/security/), [Production](/docs/production/), and
   [Troubleshooting](/docs/troubleshooting/) as an operational readiness checklist.

Evaluating another CMS? Start with [Move from Payload](/guides/from-payload/) or
[Ridu for PocketBase users](/guides/from-pocketbase/).
