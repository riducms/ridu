...
description: 'Add a media library to a Ridu project, upload files through the admin or SDK, and reference them from content.'
product: admin
eyebrow: 'Authoring'
order: 125
aliases:
  ['upload', 'media', 'file', 'image', 'multipart', 'remote URL', 'focal point', 'orphan cleanup']
availability:
  status: limited
  label: 'Bounded server upload'
  description: 'Multipart and guarded remote ingestion, image variants, delivery, and reconciliation work; resumable/direct upload, quarantine, and malware scanning are not implemented.'
  anchor: limits
navigation:
...
[Upload field guide](/docs/fields/upload/) for filtering and delete behavior.
...
![A Ridu Media document editor showing the asset preview, detected file metadata, authored alt text, image variants, and document actions.](../../../../docs/assets/ridu-admin-upload.png)
...
configure [CORS and credentialed cookies](/docs/cors/). Server-side callers can supply an API-key
...
[Object storage](/docs/storage/#s3-storage) for timeouts, local emulators, signed URLs, and the
backend contract.
...
application readiness to pass. The [Migrations guide](/docs/migrations/) covers adapter-specific
