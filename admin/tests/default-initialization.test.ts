...
import type { NotificationCenter } from "../src/core/notifications/notification-center.svelte";
import type { AdminRuntime } from "../src/core/runtime/admin-runtime.svelte";
...
const { CreateFirstUserController } =
	await import("../src/features/auth/create-first-user-controller.svelte");
const { ReferenceBrowserWorkflow } =
	await import("../src/features/reference-browser/reference-browser-workflow.svelte");
...
function multiSelectField(name: string, defaultValues: string[]): SchemaField {
	return {
		...
		select: {
			hasMany: true,
			defaultValues,
			choices: defaultValues.map((value) => ({ value, label: value })),
		},
	};
}
