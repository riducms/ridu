...
type Collection struct {
	...
	Access     map[Kind]Access
	Hooks      Hooks
	Fields     map[string]FieldRules
	FieldHooks map[string]Hooks
...
func (engine *Engine) Execute(ctx context.Context, request Request) (result Result, err error) {
	...
	if selection.All && !request.LocalizationPrepared && (request.Operation == Create || request.Operation == Duplicate || request.Operation == Update) {
	...
	if (request.Operation == Publish || request.Operation == Unpublish) && hookSelection.All {
	...
	if request.LocalizationPrepared {
		...
		projected := localization.ProjectDocument(store.Document{Values: request.Data}, collection.Schema.Fields, preparedValidationSelection)
		request.Data = projected.Values
		failureContext.Data = store.CloneValues(projected.Values)
	...
	if request.Operation != Create && request.Operation != Duplicate && request.Operation != Read && request.Operation != Update && request.Operation != Delete && request.Operation != RestoreDeleted && request.Operation != DeletePermanent && request.Operation != Publish && request.Operation != Unpublish {
		return Result{}, &Error{Code: "bad_operation", Status: 400, Message: fmt.Sprintf("operation %q is not supported", request.Operation)}
	}
	if collection.Schema.Capabilities.Global && (request.Operation == Create || request.Operation == Duplicate || request.Operation == Delete || request.Operation == RestoreDeleted || request.Operation == DeletePermanent || request.Operation == Read && request.ID == "") {
	...
	if request.TrashOnly && (request.Operation != Read || !collection.Schema.Capabilities.Trash) {
		return Result{}, &Error{Code: "bad_operation", Status: 400, Message: "trash queries require a trash-enabled collection read"}
	}
	if request.Draft != nil && request.Operation != Read && request.Operation != Create {
	...
	if request.Draft != nil && collection.Schema.Versions == nil && request.Operation != Read {
	...
	if collection.Schema.Capabilities.Upload && !request.StoragePrepared {
		if request.Operation == Create && request.ImportID == "" || request.Operation == Duplicate {
		...
		if (request.Operation == Update || request.Operation == Publish || request.Operation == Unpublish) &&
	...
	if request.Operation != Duplicate {
		...
		if decision.Kind == Deny || request.Operation == Create && decision.Kind == Where {
			return Result{}, &Error{Code: "access_denied", Status: 403, Message: "operation is not permitted"}
		}
		if (request.Operation == Publish || request.Operation == Unpublish) && hasSubmittedChanges {
		...
		if request.Operation == Create && request.Draft != nil && !*request.Draft {
			publishContext := operationContext
			publishContext.Operation = Publish
	...
	if request.Operation == Duplicate || request.Operation == Update || request.Operation == Delete || request.Operation == RestoreDeleted || request.Operation == DeletePermanent || request.Operation == Publish || request.Operation == Unpublish {
		deletion := store.DeletionActive
		if request.Operation == RestoreDeleted || request.Operation == DeletePermanent {
			deletion = store.DeletionTrash
		}
		findAccess := decision.Access
		if request.Operation == Duplicate {
			readContext := operationContext
			readContext.Operation, readContext.Data = Read, store.Values{}
		...
		if findError != nil {
			if collection.Schema.Capabilities.Global && request.Operation == Update && decision.Kind == Allow && errors.Is(findError, store.ErrNotFound) {
			...
			if request.Operation == Update && collection.Schema.Versions != nil &&
			...
			if request.LocalizationPrepared || request.Operation == Publish || request.Operation == Unpublish {
				visibleSelection = hookSelection
			}
			visibleOriginal := localization.ProjectDocument(canonicalDocument, collection.Schema.Fields, visibleSelection)
			originalVisible = store.CloneValues(visibleOriginal.Values)
			operationContext.Original = cloneDocumentPointer(&visibleOriginal)
			if request.Operation == Delete || request.Operation == DeletePermanent || request.Operation == RestoreDeleted {
				request.Data, operationContext.Data = store.CloneValues(visibleOriginal.Values), store.CloneValues(visibleOriginal.Values)
			} else if request.Operation == Publish || request.Operation == Unpublish {
			...
			if request.Operation == Duplicate {
				duplicateCanonical = store.CloneValues(originalCanonical)
				duplicateVisible = store.CloneValues(visibleOriginal.Values)
				values := store.CloneValues(visibleOriginal.Values)
				for name, value := range request.Data {
					values[name] = value
				}
				request.Data, operationContext.Data = values, store.CloneValues(values)
				candidate, candidateError := duplicateCanonicalValues(collection.Schema.Fields, duplicateCanonical, duplicateVisible, values, selection)
				...
				for _, locale := range createLocales {
					...
					if locale != "" {
						projected := localization.ProjectDocument(store.Document{Values: candidate}, collection.Schema.Fields, localization.Selection{Locale: locale, Chain: []schema.LocaleCode{locale}, Configured: selection.Configured})
						createContext.Data = projected.Values
					}
	...
	if request.Operation == Duplicate {
		if err := runHooks(collection.Hooks.BeforeDuplicate, operationContext); err != nil {
			return Result{}, hookError("before duplicate", err)
		}
		if err := runFieldHooks(collection.FieldHooks, operationContext, func(hooks Hooks) []Hook { return hooks.BeforeDuplicate }); err != nil {
	...
	if request.Operation == Delete || request.Operation == DeletePermanent {
		if err := runHooks(collection.Hooks.BeforeDelete, operationContext); err != nil {
			return Result{}, hookError("before delete", err)
		}
		if err := runFieldHooks(collection.FieldHooks, operationContext, func(hooks Hooks) []Hook { return hooks.BeforeDelete }); err != nil {
			return Result{}, hookError("field before delete", err)
		}
	}

	if err := runHooks(collection.Hooks.BeforeValidate, operationContext); err != nil {
		return Result{}, hookError("before validation", err)
	}
	if err := runFieldHooks(collection.FieldHooks, operationContext, func(hooks Hooks) []Hook { return hooks.BeforeValidate }); err != nil {
	...
	if request.Operation == Duplicate {
	...
	} else if request.Operation == Update || request.Operation == Publish || request.Operation == Unpublish {
		...
		candidate := localization.ProjectDocument(store.Document{Values: candidateCanonical}, collection.Schema.Fields, writeSelection)
		fieldAuthorizationAfter = candidate.Values
		if originalCanonicalDocument != nil {
			before := localization.ProjectDocument(*originalCanonicalDocument, collection.Schema.Fields, writeSelection)
			fieldAuthorizationBefore = before.Values
		}
	}
	fieldAuthorizationOperation := request.Operation
	if fieldAuthorizationOperation == Publish || fieldAuthorizationOperation == Unpublish {
		fieldAuthorizationOperation = Update
	}
	if !request.SkipFieldAccess {
		if err := authorizeFields(collection, fieldAuthorizationContext, fieldAuthorizationOperation, fieldAuthorizationBefore, fieldAuthorizationAfter, request.LocalizationPrepared); err != nil {
			return Result{}, err
		}
	}
	if request.Operation == Create || request.Operation == Duplicate || request.Operation == Update || request.Operation == Publish || request.Operation == Unpublish {
		normalizeAuthIdentity(collection.Schema, request.Data)
		validated, validationIssues := validate(collection.Schema.Fields, request.Data, request.Operation == Create || request.Operation == Duplicate || originalMissing, engine.pluginValidators)
	...
	if changesDocument(request.Operation) {
		if err := runHooks(collection.Hooks.BeforeChange, operationContext); err != nil {
			return Result{}, hookError("before change", err)
		}
		if err := runFieldHooks(collection.FieldHooks, operationContext, func(hooks Hooks) []Hook { return hooks.BeforeChange }); err != nil {
			return Result{}, hookError("field before change", err)
		}
	}
	if err := runHooks(collection.Hooks.BeforeOperation, operationContext); err != nil {
		return Result{}, hookError("before operation", err)
	}
	if err := runFieldHooks(collection.FieldHooks, operationContext, func(hooks Hooks) []Hook { return hooks.BeforeOperation }); err != nil {
	...
	} else if request.Operation == RestoreDeleted {
	...
	validateExistingUploadObjects := collection.Schema.Capabilities.Upload &&
		(request.Operation == Create && request.ImportID != "" || request.ValidateUploadObjects)
	...
	if request.Operation == Create || request.Operation == Duplicate || request.Operation == Update || request.Operation == RestoreDeleted || request.Operation == Publish || request.Operation == Unpublish {
	...
	hardDelete := request.Operation == DeletePermanent || request.Operation == Delete && !collection.Schema.Capabilities.Trash
	if hardDelete {
	...
	if request.Operation != Read && result.Document != nil && len(request.Populate) != 0 &&
		request.Operation != DeletePermanent &&
		!(request.Operation == Delete && !collection.Schema.Capabilities.Trash) {
	...
	if result.Document != nil {
		...
		hookDocument := localization.ProjectDocument(*hookSource, collection.Schema.Fields, hookSelection)
	...
	if result.Document != nil {
		projected := localization.ProjectDocument(*result.Document, collection.Schema.Fields, selection)
		result.Document = &projected
	}
	if result.Page != nil {
		for index := range result.Page.Documents {
			result.Page.Documents[index] = localization.ProjectDocument(result.Page.Documents[index], collection.Schema.Fields, selection)
		}
	}
	...
	if changesDocument(request.Operation) {
		...
		if err := runFieldHooks(collection.FieldHooks, operationContext, func(hooks Hooks) []Hook { return hooks.AfterChange }); err != nil {
	...
	if request.Operation == Delete || request.Operation == DeletePermanent {
		...
		if err := runFieldHooks(collection.FieldHooks, operationContext, func(hooks Hooks) []Hook { return hooks.AfterDelete }); err != nil {
	...
	if err := runFieldHooks(collection.FieldHooks, operationContext, func(hooks Hooks) []Hook { return hooks.AfterOperation }); err != nil {
	...
	for _, path := range sortedFieldHookPaths(collection.FieldHooks) {
		hooks := collection.FieldHooks[path]
		for _, hook := range hooks.AfterCommit {
			fieldContext := operationContext
			fieldContext.FieldPath = path
			state.afterCommit = append(state.afterCommit, deferredHook{hook: hook, context: fieldContext})
		}
	}
	for _, hook := range collection.Hooks.AfterCommit {
		state.afterCommit = append(state.afterCommit, deferredHook{hook: hook, context: operationContext})
	}
...
func importedUploadObjectKeys(values store.Values) ([]string, error) {
	...
	if sizes, valid := values["sizes"].ObjectValue(); valid {
		if len(sizes) > store.MaxUploadReferenceCandidates-1 {
		...
		for _, encoded := range sizes {
			...
			if key, valid := metadata["objectKey"].StringValue(); valid && key != "" {
				unique[key] = struct{}{}
			}
		}
	}
...
func recordsVersion(operation Kind) bool {
	switch operation {
	case Create, Duplicate, Update, Publish, Unpublish:
		return true
	default:
		return false
	}
...
func removeDuplicateValues(fields []schema.Field, before, after store.Values, selection localization.Selection, values store.Values) {
	...
	for name := range before {
		...
		byLocale, valid := values[name].ObjectValue()
		if !valid {
			delete(values, name)
			continue
		}
...
func (engine *Engine) readVersions(ctx context.Context, collectionName, documentID string, revision int, actor *store.Document, localeOptions ...LocalizationOptions) (versions []store.Version, err error) {
	...
	stack, _ := ctx.Value(operationStackKey{}).([]operationFrame)
	frame := operationFrame{kind: ReadVersions, collection: collectionName, id: documentID}
	if len(stack) >= engine.maxDepth || repeatedFrame(stack, frame) >= 4 {
	...
	operationContext := Context{Context: transactionContext, Operation: ReadVersions, Collection: collection.Schema, ID: documentID, Actor: cloneDocumentPointer(actor), ActorCollection: options.ActorCollection, Data: store.Values{}, Locale: selection.Locale, AllLocales: selection.All, Locales: append([]schema.LocaleCode(nil), selection.Configured...)}
	defer func() {
		if err == nil {
			return
		}
	...
	for _, path := range sortedFieldHookPaths(collection.FieldHooks) {
		for _, hook := range collection.FieldHooks[path].AfterCommit {
			fieldContext := operationContext
			fieldContext.FieldPath = path
			state.afterCommit = append(state.afterCommit, deferredHook{hook: hook, context: fieldContext})
		}
...
func (engine *Engine) RestorePopulated(ctx context.Context, collectionName, documentID string, revision, expectedRevision int, draft bool, actor *store.Document, populations []query.Population, outputFields []query.Path, localeOptions ...LocalizationOptions) (result Result, err error) {
	...
	versionContext := Context{
		Context: transactionContext, Operation: ReadVersions, Collection: collection.Schema,
	...
	result, err = engine.Execute(transactionContext, Request{
		Operation: operation, Collection: collectionName, ID: documentID, Data: version.Snapshot.Values,
...
func authorizeFields(collection Collection, operationContext Context, kind Kind, beforeValues, afterValues store.Values, authorizeEveryUpdateValue bool) error {
	if kind != Create && kind != Duplicate && kind != Update {
		return nil
	}
	paths := sortedFieldRulePaths(collection.Fields)
	for _, path := range paths {
		...
		locations := fieldLocationsAtPath(collection.Schema.Fields, afterValues, path, operationContext.AllLocales)
		if kind == Update {
			beforeLocations := fieldLocationsAtPath(collection.Schema.Fields, beforeValues, path, operationContext.AllLocales)
			changed := changedFieldLocations(beforeLocations, locations)
			if authorizeEveryUpdateValue {
				locations = append(locations, removedFieldLocations(changed, locations)...)
			} else {
				locations = changed
			}
		}
		for _, location := range locations {
			fieldContext := operationContext
			fieldContext.FieldPath, fieldContext.RuntimePath = path, location.runtimePath
			fieldContext.Value, fieldContext.SiblingData = location.value, location.siblings
			if location.locale != "" {
				fieldContext.Locale = location.locale
			}
			if operationContext.AllLocales && location.locale != "" {
				projectedData := localization.ProjectDocument(store.Document{Values: operationContext.Data}, collection.Schema.Fields, localization.Selection{
					Locale: location.locale, Chain: []schema.LocaleCode{location.locale}, Configured: append([]schema.LocaleCode(nil), operationContext.Locales...),
				})
				fieldContext.Data = projectedData.Values
			}
			fieldContext.Original = fieldLocaleDocument(operationContext.Original, collection.Schema.Fields, operationContext, location.locale)
			fieldContext.Document = cloneDocumentPointer(fieldContext.Original)
...
func (engine *Engine) redactResult(collection Collection, operationContext Context, result *Result) error {
	...
	redact = func(current Collection, currentContext Context, document *store.Document) error {
		...
		for _, path := range sortedFieldRulePaths(current.Fields) {
			rules := current.Fields[path]
			if rules.Read == nil {
				continue
			}
			for _, location := range fieldLocationsAtPath(current.Schema.Fields, document.Values, path, currentContext.AllLocales) {
				...
				fieldContext.Document = fieldLocaleDocument(document, current.Schema.Fields, currentContext, location.locale)
				allowed, err := rules.Read(fieldContext)
				if err != nil {
					return err
				}
				if !allowed {
...
func valueExistsAtRuntimePath(values store.Values, path string) bool {
	...
	for _, segment := range segments[1:] {
		if object, valid := current.ObjectValue(); valid {
	...
	resolve := func(document *store.Document) error {
		for _, candidate := range collection.Schema.Fields {
			...
			case schema.FieldTypeJoin:
				...
				joinContext.Operation, joinContext.Collection, joinContext.ID = Read, target.Schema, ""
...
func runFieldHooks(fieldHooks map[string]Hooks, ctx Context, selectHooks func(Hooks) []Hook) error {
	for _, path := range sortedFieldHookPaths(fieldHooks) {
		hooks := fieldHooks[path]
		fieldContext := ctx
		fieldContext.FieldPath = path
		if err := runHooks(selectHooks(hooks), fieldContext); err != nil {
			return err
		}
	}
	return nil
...
func sortedFieldHookPaths(fieldHooks map[string]Hooks) []string {
	paths := make([]string, 0, len(fieldHooks))
	for path := range fieldHooks {
...
func runReadHooks(collection Collection, operationContext Context, result *Result) error {
	run := func(document *store.Document) error {
		...
		if err := runFieldHooks(collection.FieldHooks, readContext, func(hooks Hooks) []Hook { return hooks.AfterRead }); err != nil {
...
func changesDocument(kind Kind) bool {
	return kind == Create || kind == Duplicate || kind == Update || kind == Publish || kind == Unpublish
}
...
type fieldLocation struct {
	value       store.Value
	siblings    store.Values
	runtimePath string
	identity    string
	locale      schema.LocaleCode
}

func fieldLocaleDocument(document *store.Document, fields []schema.Field, base Context, locale schema.LocaleCode) *store.Document {
	if document == nil {
		return nil
	}
	if !base.AllLocales || locale == "" {
		return cloneDocumentPointer(document)
	}
	selection := localization.Selection{
		Locale: locale, Chain: []schema.LocaleCode{locale},
		Configured: append([]schema.LocaleCode(nil), base.Locales...),
	}
	projected := localization.ProjectDocument(*document, fields, selection)
	return &projected
}

func fieldLocationSiblings(fields []schema.Field, values store.Values, allLocales bool, locale schema.LocaleCode) store.Values {
	if !allLocales || locale == "" {
		return store.CloneValues(values)
	}
	projected := localization.ProjectDocument(store.Document{Values: values}, fields, localization.Selection{
		Locale: locale, Chain: []schema.LocaleCode{locale},
	})
	return projected.Values
}

func fieldLocationsAtPath(fields []schema.Field, values store.Values, path string, allLocales bool) []fieldLocation {
	return collectFieldLocations(fields, values, path, "", "", allLocales, "")
}

func collectFieldLocations(fields []schema.Field, values store.Values, path, runtimePrefix, identityPrefix string, allLocales bool, locale schema.LocaleCode) []fieldLocation {
	for _, field := range fields {
			...
			for _, code := range locales {
				...
				if path == canonicalPath {
					locations = append(locations, fieldLocation{
						value: localized[code], siblings: fieldLocationSiblings(fields, values, allLocales, localizedLocale),
						runtimePath: localizedPath, identity: localizedIdentity, locale: localizedLocale,
					})
					continue
				}
				locations = append(locations, collectFieldDescendantLocations(field, localized[code], path, localizedPath, localizedIdentity, allLocales, localizedLocale)...)
			}
			return locations
		}
		if path == canonicalPath {
			return []fieldLocation{{value: value, siblings: fieldLocationSiblings(fields, values, allLocales, locale), runtimePath: runtimePath, identity: identityPath, locale: locale}}
		}
		return collectFieldDescendantLocations(field, value, path, runtimePath, identityPath, allLocales, locale)
	}
	return nil
}

func collectFieldDescendantLocations(field schema.Field, value store.Value, path, runtimePath, identityPath string, allLocales bool, locale schema.LocaleCode) []fieldLocation {
	switch field.Type {
	case schema.FieldTypeGroup:
		...
		return collectFieldLocations(field.Nested.Fields, object, path, runtimePath, identityPath, allLocales, locale)
	case schema.FieldTypeArray:
		...
		var locations []fieldLocation
		keyOccurrences := make(map[string]int, len(items))
		for index, item := range items {
			...
			if key, valid := object["_key"].StringValue(); valid && key != "" {
				rowIdentity = keyedRowIdentity(key, keyOccurrences[key])
				keyOccurrences[key]++
			}
			locations = append(locations, collectFieldLocations(
				field.Nested.Fields, object, path, fmt.Sprintf("%s.%d", runtimePath, index), joinFieldPath(identityPath, rowIdentity), allLocales, locale,
			)...)
		}
		return locations
	case schema.FieldTypeBlocks:
		...
		for index, item := range items {
			...
			for _, block := range field.Blocks.Types {
				...
				locations = append(locations, collectFieldLocations(
					block.Fields, object, path, fmt.Sprintf("%s.%d", runtimePath, index), joinFieldPath(identityPath, rowIdentity), allLocales, locale,
				)...)
				break
			}
		}
...
func deleteValueAtRuntimeSegments(values store.Values, segments []string) {
	...
	object, valid := items[index].ObjectValue()
	if !valid {
		return
	}
...
func sortedFieldRulePaths(rules map[string]FieldRules) []string {
...
func authorize(collection Collection, ctx Context) (Decision, error) {
	...
	if kind == Duplicate {
		kind = Create
	} else if kind == RestoreDeleted || kind == DeletePermanent {
		kind = Delete
	}
...
func authorizePreparedLocalization(collection Collection, base Context, canonical store.Values, locales []schema.LocaleCode) (Decision, error) {
	...
	for index, locale := range locales {
		...
		projected := localization.ProjectDocument(store.Document{Values: canonical}, collection.Schema.Fields, selection)
		current := base
		current.Data = projected.Values
		current.Locale = locale
