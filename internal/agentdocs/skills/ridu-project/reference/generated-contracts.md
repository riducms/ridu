...
[Schema and identifiers](./go-packages/schema.md) explains how this manifest differs from
...
database connection in your Go config. See [Project structure](https://riducms.com/guides/project-structure/) for
...
Sort terms are strings; see [Querying data](./querying.md) for examples.
...
create request. Use [validation](./fields/validation.md) when allowed values depend on another
field, and [access rules](./access-control.md) to protect them.
...
the database from the previous schema to the new one. Read [Migrations](./migrations.md) before
...
See [Troubleshooting](./troubleshooting.md) for more help, the
[schema reference](https://riducms.com/reference/schema/) for schema types, and the [CLI reference](https://riducms.com/reference/cli/)
for generation commands.

## Primitive list contracts {#primitive-lists}

[TextList and NumberList](./fields/lists.md) generate Go slices and TypeScript `string[]` or
