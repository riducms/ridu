<!-- Generated from website/src/content/guides/custom-fields.md by scripts/sync-agent-docs.ts. -->

# Build a field plugin
...
[Field components](./custom-components/field-components.md). That tutorial works directly in
your application's `admin.config.ts` and does not require a plugin. Built-in
[TextList and NumberList](./fields/lists.md) editors also use this path: register
...
- [`definePluginField`](https://riducms.com/reference/plugin/define-plugin-field/) connects `ColorField` to its value
...
- [`defineAdminPlugin`](https://riducms.com/reference/plugin/define-admin-plugin/) collects those field registrations
...
its migrations on each supported database. See [Testing](./testing.md) and [Plugins](./plugins.md).
