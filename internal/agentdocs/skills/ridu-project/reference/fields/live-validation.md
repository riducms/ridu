<!-- Generated from website/src/content/docs/fields/live-validation.md by scripts/sync-agent-docs.ts. -->
...
| Enforce your own rule when saving                     | [`.Validate(...)`](./validation.md)                 |
...
For wrappers and accessors, see [Operations and callbacks](../go-packages/operation.md) and
[Documents and values](../go-packages/store.md).
...
[`LiveValidationContext` reference](https://riducms.com/reference/operation/live-validation-context/) for every
property and [Using other field values](./callback-values.md) for nested reads.
...
edit. Use [hooks](../hooks.md) for work tied to saving.
...
child or row with the same [`operation.Issue` targets](./validation.md#validation-issue-targets)
...
The framework also handles live checking for [custom field components](../custom-components/field-components.md)
and [plugin fields](../custom-fields.md). Continue to update the value through `field.set(...)`
...
[`FieldLiveValidation`](https://riducms.com/reference/plugin/field-live-validation/) for the contract.
...
[`TextList` and `NumberList`](./lists.md#live-validation) use the same opt-in API.
