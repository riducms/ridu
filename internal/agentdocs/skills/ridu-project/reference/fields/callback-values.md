<!-- Generated from website/src/content/docs/fields/callback-values.md by scripts/sync-agent-docs.ts. -->
...
[Operations and callbacks](../go-packages/operation.md). [Documents and values](../go-packages/store.md)
...
[field access rules](../access-control.md#field-access) if only certain users should be able
to change the lock.
...
[collection or global hook](../hooks/context.md#hook-context) when a change spans several fields.
...
would skip these new values. See [Set default field values](./defaults.md#context) for
...
[localization source metadata](../localization.md#read-one) when you need to know its source.
...
[a collection or global hook with the local API](../hooks/transactions-and-errors.md#nested-operations).
...
The [Live server validation guide](./live-validation.md) explains these properties with
