<!-- Generated from website/src/content/docs/fields/defaults.md by scripts/sync-agent-docs.ts. -->
...
| Change an existing value when saving                   | A [field hook](../hooks/fields.md)      | Trim a title each time it changes                          |
| Calculate a value for each response without storing it | A [virtual field](https://riducms.com/docs/fields/virtual/) | Calculate a reading time                                   |
...
example assumes you have [configured English and French content locales](../localization.md).
...
For [rich-text embedded forms](../rich-text.md), **Apply** updates the rich-text content in the
...
[Operations and callbacks](../go-packages/operation.md#value-presence) for these wrappers.
...
[editing nested data](../go-packages/store.md#nested-values) for a complete update example.
...
example for a unique field—a [BeforeDuplicate hook](../hooks/collections.md) can remove that
...
Use a [document hook](../hooks/collections.md) when several values must be calculated in order.
...
children. See [Reusable fields](../fields.md#field-access-and-hooks) for those patterns.
...
[Generated types and files](../generated-contracts.md).
