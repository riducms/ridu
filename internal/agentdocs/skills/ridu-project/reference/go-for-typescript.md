<!-- Generated from website/src/content/docs/go-for-typescript.md by scripts/sync-agent-docs.ts. -->
...
- Put lifecycle behaviour in [hooks](./hooks.md) and keep side effects safe for retries.
- Use the [local API](./local-api.md) for trusted Go callers; it still passes through the same
  authorization, validation, hooks, transaction, and redaction engine as REST.
- Use the generated TypeScript module or [REST API](./rest-api.md) from application frontends.
...
1. Follow [Getting started](https://riducms.com/docs/getting-started/) and read the generated project.
2. Add one field with [Collections](./collections.md) and [Fields](./fields.md).
3. Run generation and use the new type from [TypeScript SDK](./typescript-sdk.md).
