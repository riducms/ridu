<!-- Generated from website/src/content/docs/configuration.md by scripts/sync-agent-docs.ts. -->
...
See the [`ridu.Config` reference](https://riducms.com/reference/ridu/config/) for every field.
...
admin but do not grant authorization. See [Admin](./admin.md) and
[Authentication](https://riducms.com/docs/authentication/) for those separate concerns.
...
the [schema reference](https://riducms.com/reference/schema/locale-code/).
...
[SQLite](./sqlite.md). MongoDB-selected projects import `adapters/mongodb`, open `DATABASE_URL`,
and verify the resolved index plan before serving; see [MongoDB](./mongodb.md).
...
See [Ridu CLI](https://riducms.com/docs/cli/) for the development and release commands.
