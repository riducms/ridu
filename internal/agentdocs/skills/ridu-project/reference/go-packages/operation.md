<!-- Generated from website/src/content/docs/go-packages/operation.md by scripts/sync-agent-docs.ts. -->
...
use the [local Go API](../local-api.md). For the values in a document's map, see
[Document values](./store.md).
...
does not mean every hook runs for it. See the [hook lifecycle](../hooks/collections.md#lifecycle)
...
for omitted fields that need an initial value. See [Set default field values](../fields/defaults.md).
...
See [missing input and null](../fields/callback-values.md#missing-input) for more detail.
...
[Field hooks](../hooks/fields.md) for both kinds of hook and their registration.
...
[Using other field values](../fields/callback-values.md) for nested rows, previous values,
...
fields or related writes. [Hook context](../hooks/context.md) explains when values are
...
For nested lists and translations, see [validation targets](../fields/validation.md#rows).
For additional types and methods, see the [operation reference](https://riducms.com/reference/operation/).
...
Use the [Live server validation guide](../fields/live-validation.md) for commented examples,
