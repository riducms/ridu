<!-- Generated from website/src/content/docs/production.md by scripts/sync-agent-docs.ts. -->
...
Start with [Build and run](./production/build-and-run.md) to produce the binary and launch it on
your own machine. Use [Deploy to Railway](./production/railway.md) for a complete hosted example.
...
> recovery guidance below does not widen that envelope. See [SQLite](./sqlite.md) for its WAL-safe
...
> [MongoDB](./mongodb.md) for the exact boundary.
...
6. Follow the [abuse-prevention checklist](https://riducms.com/docs/preventing-abuse/) for access rules, authentication
...
[Prevent abuse](https://riducms.com/docs/preventing-abuse/) for the public-traffic checklist and
[Security](https://riducms.com/docs/security/) for the full trust boundary and request-work ceilings.
...
- `JobError` receives worker and scheduled-operation infrastructure failures; handler retry state is
  also queryable through [durable tasks](https://riducms.com/docs/tasks/).
...
Review [Releases and compatibility](https://riducms.com/docs/releases/) for supported platforms and upgrade policy,
[PostgreSQL](./postgres.md) for pools and transactions, [SQLite](./sqlite.md) for the embedded
operating envelope, [MongoDB](./mongodb.md) for its bounded replica-set profile, and
[Object storage](https://riducms.com/docs/storage/) for backend and reconciliation requirements.
