...
func TestCallbackViewsDetachOwnedMaps(t *testing.T) {
	...
	if got := view.Get("absent").Kind(); got != store.ValueNull {
		t.Fatalf("absent prototype snapshot value kind = %q; want null", got)
	}
}
...
func TestRelativeIssueUsesExistingImmutablePath(t *testing.T) {
	segments := []string{"label"}
	path, err := query.NewPath(segments...)
	if err != nil {
		t.Fatal(err)
	}
	issue := operation.Issue{Code: "invalid_label", Message: "Choose a label.", Path: path}
	segments[0] = "mutated"
	returned := issue.Path.Segments()
	returned[0] = "changed"
