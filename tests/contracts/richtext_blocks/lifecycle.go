...
func parentLifecycle(t *testing.T, factory Factory) {
	...
	ctx := t.Context()
	user, err := app.Local().Create(ctx, "users", store.Values{"email": store.String("publisher@example.test")}, nil)
	if err != nil {
		t.Fatal(err)
	}
	asset, err := app.Local().Create(ctx, "assets", store.Values{"title": store.String("Mountains"), "url": store.String("/mountains.svg")}, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := app.Local().Find(ctx, "articles", created.ID, nil); err == nil {
	...
	published, err := app.Local().Publish(ctx, "articles", created.ID, created.Revision, &user)
	if err != nil {
		t.Fatal(err)
	}
	...
	changed, err := app.Local().PublishChanges(ctx, "articles", created.ID, store.Values{"body": Document(Block("callout", "callout", store.Values{"title": store.String("Changed title")}))}, published.Revision, &user)
	if err != nil {
		t.Fatal(err)
	}
	restored, err := app.Local().RestoreAsDraft(ctx, "articles", created.ID, published.Revision, changed.Revision, &user)
	if err != nil {
		t.Fatal(err)
	}
	...
	duplicate, err := app.Local().Duplicate(ctx, "articles", created.ID, store.Values{"title": store.String("Copy")}, &user)
	if err != nil {
		t.Fatal(err)
	}
	...
	public, err := app.Local().Find(ctx, "articles", created.ID, nil)
	...
	versions, err := app.Local().Versions(ctx, "articles", created.ID, &user)
